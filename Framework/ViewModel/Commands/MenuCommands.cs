...
namespace Framework.ViewModel
{
    public class MenuCommands : BaseVM
    {
        ...
        private ICommand _threshHolding;
        public ICommand ThreshHoldingCommand
        {
            get
            {
                if (_threshHolding == null)
                    _threshHolding = new RelayCommand(ThreshHoldImage);
                return _threshHolding;
            }
        }
        private void ThreshHoldImage(object parameter)
        {
            if (InitialImage == null)
            {
                MessageBox.Show("Please add an image !");
            ...
            byte prag = ((byte)values[0]);
            if (GrayInitialImage != null)
            {
                GrayProcessedImage = Tools.ThreshHold(GrayInitialImage,prag);
                ProcessedImage = Convert(GrayProcessedImage);
            }
            else if (ColorInitialImage != null)
            {
             //   convert to gray
                Image<Gray, byte> grayImage = Tools.Convert(ColorInitialImage);
                grayImage = Tools.ThreshHold(grayImage, prag);
                ProcessedImage = Convert(grayImage);
            }
        }
        #endregion
        #region Crop Image

        private ICommand _cropImage;
        public ICommand CropImageCommand
        {
            get
            {
                if (_cropImage == null)
                    _cropImage = new RelayCommand(CropImage);
                return _cropImage;
            }
        }
        private void CropImage(object parameters)
        {
            if(InitialImage == null)
            {
                MessageBox.Show("Please add an image !");
            ...
            Canvas initialCanvas = param[0] as Canvas;
            Canvas processedCanvas = param[1] as Canvas;
            Point firstClick= VectorOfMousePosition[VectorOfMousePosition.Count-2];
            Point lastClick = VectorOfMousePosition[VectorOfMousePosition.Count - 1];
            ...
            {
                (firstClick, lastClick) = (lastClick, firstClick);
            }
            ClearProcessedCanvas(processedCanvas);
            if(GrayInitialImage!= null)
            {
                RemoveUiElements(initialCanvas);
                DrawRectangle(initialCanvas, firstClick, lastClick, 2, Brushes.Red, ScaleValue);
                GrayProcessedImage = Tools.Crop(GrayInitialImage, Tuple.Create(firstClick.X, firstClick.Y), Tuple.Create(lastClick.X,lastClick.Y));
                ProcessedImage = Convert(GrayProcessedImage);
            }
            if(ColorInitialImage!= null)
            {
                RemoveUiElements(initialCanvas);
                DrawRectangle(initialCanvas, firstClick, lastClick, 2, Brushes.Red, ScaleValue);
                ColorProcessedImage = Tools.CropColor(ColorInitialImage, Tuple.Create(firstClick.X, firstClick.Y), Tuple.Create(lastClick.X, lastClick.Y));
                ProcessedImage=Convert(ColorProcessedImage);
            }
        }
        #endregion
        #region Mirror Image
        private ICommand _mirrorImage;

        public ICommand MirrorImageCommand
        {
            get
            {
                if (_mirrorImage == null)
                    _mirrorImage = new RelayCommand(Mirror);
                return _mirrorImage;
            }
        }
        ...
        private ICommand _rotateClockwiseImage;
        public ICommand RotateClockWiseImageCommand
        {
            get
            {
                if (_rotateClockwiseImage == null)
                {
                    _rotateClockwiseImage = new RelayCommand(RotateClockwise);
                }
                return _rotateClockwiseImage;
            }
        }
        ...
        private ICommand _rotateCounterClockwiseImage;
        public ICommand RotateCounterClockWiseImageCommand
        {
            get
            {
                if (_rotateCounterClockwiseImage == null)
                {
                    _rotateCounterClockwiseImage = new RelayCommand(RotateCounterClockwise);
                }
                return _rotateCounterClockwiseImage;
            }
        }
        private void RotateCounterClockwise(object parameter)
        {
            ...
            if (ColorInitialImage != null)
            {
                ColorProcessedImage = Tools.RotateAntiClockwisw(ColorInitialImage);
                ProcessedImage = Convert(ColorProcessedImage);
            }

            if (GrayInitialImage != null)
            {
                GrayProcessedImage = Tools.RotateAntiClockwisw(GrayInitialImage);
                ProcessedImage = Convert(GrayProcessedImage);
            }
        }
        #endregion
